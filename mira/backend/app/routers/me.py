...
def _app_entries(eid: int) -> pd.DataFrame:
    rows = mongo.find_ledger_entries_by_enterprise(eid)
    if not rows:
        return pd.DataFrame(columns=["date", "kind", "amount", "category", "note"])
    df = pd.DataFrame([{
        "date": pd.Timestamp(r["date"]), "kind": r["kind"], "amount": r["amount"],
        "category": r["category"], "note": r["note"], "client_uuid": r.get("client_uuid"),
    } for r in rows])
    return df
...
@router.get("/summary")
def summary(user: User = Depends(require_enterprise)):
    eid = _eid(user)
...
@router.get("/forecast")
def forecast(user: User = Depends(require_enterprise)):
    eid = _eid(user)
    history = store.monthly_history(eid, months=12, extra_entries=_app_entries(eid))
...
def _validate_entry(eid: int, e: LedgerIn) -> str | None:
...
def _insert_entry(eid: int, e: LedgerIn) -> dict:
    if e.client_uuid:
        dup = mongo.find_ledger_entry_by_client_uuid(e.client_uuid)
        if dup is not None:
            return {"id": mongo.ledger_entry_id(dup), "client_uuid": e.client_uuid, "deduped": True}
    reason = _validate_entry(eid, e)
    if reason is not None:
        return {"client_uuid": e.client_uuid, "rejected": reason}
    doc = mongo.insert_ledger_entry(enterprise_id=eid, date=e.date, kind=e.kind,
                                    category=e.category, amount=e.amount, note=e.note,
                                    client_uuid=e.client_uuid)
    return {"id": mongo.ledger_entry_id(doc), "client_uuid": e.client_uuid, "deduped": False}


@router.post("/ledger")
def add_ledger(body: LedgerIn, user: User = Depends(require_enterprise)):
    res = _insert_entry(_eid(user), body)
    if res.get("rejected"):
...
def add_ledger_batch(body: LedgerBatchIn, user: User = Depends(require_enterprise)):
    eid = _eid(user)
    results = [_insert_entry(eid, e) for e in body.entries]
